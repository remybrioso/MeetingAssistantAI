...
def test_setup_wizard_reports_ready_when_all_capabilities_are_available() -> None:
    ...
    assert result.status == SetupWizardStatus.READY
    assert result.is_ready is True
    assert result.can_continue is True
    assert result.is_blocked is False

    assert result.details["available_count"] == 3
