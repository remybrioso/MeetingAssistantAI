...
def test_setup_wizard_reports_attention_when_audio_is_degraded() -> None:
    ...
    assert result.status == SetupWizardStatus.ATTENTION
    assert result.can_continue is True
    assert result.has_attention_items is True
    assert result.is_ready is False
    assert result.is_blocked is False

    assert result.details["degraded_count"] == 1
    assert len(result.get_degraded()) == 1
